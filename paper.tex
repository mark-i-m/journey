...
\title{Journey: A Study of Scalability of Memory Management in Linux}
...
sets up a page table entry for a valid access. This is called \textit{demand paging}.
...
4KB page. For each of these swap spaces, the kernel keeps a swap map, which records
...
overhead experiments. It simply prints the $\Delta_i$ for each $i$ while performing nops. Its graph
...
\begin{figure}[t]
    ...
    \caption{$\Delta_i$ as defined in section \ref{ss_vm_area_struct}. Note that
...
The \texttt{rand} workload, on the other hand, writes to pages that are 
not subpages of the same huge page. As a result, a huge page
...
\begin{figure}[t]
    ...
    \end{tabular}
    \caption {Tail latency of memory accesses.\label{fig:latency_table}}
\end{figure}

This experiment is designed to measure the impact of swapping on memory
operation latency.

In the \texttt{rand} benchmark, as before, all
memory is exhausted, and swapping begins very soon. Many memory accesses 
thus take milliseconds to complete
...
time; they impact performance significantly. Figure \ref{fig:latency_table} summarizes
this trend.
...
through phases of maintaining LRU lists and reclaiming pages from
them. This repeats until the amount of free memory increases
...
by kswapd. Moreover, in some cases,
...
created with disk locality in mind. 
...
Our results also show that in some cases, kswapd induces more overhead choosing 
a good page to reclaim than would occur by reclaiming the wrong page. Most of this 
overhead seems to be due to the repitition fo cheap operations millions of times. 

Systems with larger memory may conceivably support more memory overcommitment,
so improving the performance of swapping may be highly relevant moving forward.
Additionally, in systems with orders of magnitude of more pages, kswapd may spend
significantly more time maintaining LRU lists.
...
In the \texttt{rand} benchmark with hugepages enabled, we observe an overhead
of roughly 0.0002 jiffies per page of memory touched after swapping begins.
Also, kswapd runs for longer periods of time with longer intervals of sleep in
between. This is most likely because the benchmark has some probability of
retouching a page, so free memory is used more slowly, but kswapd also has to
do more work to find pages that have not been recently used.

In the \texttt{cont} benchmark with hugepages disabled, kswapd uses
0.0572 jiffies per page of memory touched.
...
The \texttt{cont} benchmark with hugepages enabled spends
...
operations take hundreds of milliseconds to complete, but we have not yet
verified it.
...
run more often during page faults than in its own context, causing very poor
...
New memory technologies are making it possible to have
terabytes of memory. Recent changes to memory management in Linux demonstrate
focus on improving the scalability of its data structures
...
RadixVM tries to mitigate performance issues in highly
...
actually use all available memory and swapping causes disk latency to become the main
...
generate data on the fly. This design is based on the observation that
...
the contents of a process's heap and only storing kernel data structures and
process code and stack segments. However, the common use of custom data
structures makes generating heap contents on the fly
difficult. Moreover, on Linux, the kernel data structures for 1TB
of memory may also exceed the size of physical memory on current systems
\cite{simics}. As our results show, in some cases, the overheads of
\texttt{vm\_area\_struct} alone could use significant amounts of memory.
...
benchmarks impractical. Our findings indicate several areas of improvement that
are needed before such an approach could be practical.
...
\section{Conclusion}
Terabyte-scale memory systems are poised to become commonplace in the not too distant future.
Developers and researchers need tools to study the behavior of such systems before they are available. 

In this paper, we study the scalability of the memory management 
system of Linux, which we believe is the first step toward building such tools. In particular, 
we look at the scalability of \texttt{vm\_area\_struct}  
and the swapping subsystem across a wide range of varying parameters
and workloads. 

Our findings indicate that Linux's use of
\texttt{vm\_area\_struct} tends to be scalable in the general case, but
...
While priliminary, we believe that our study is a first step towards building
tools to study large memory systems in a principled and informed manner. As our
results show, approaches similar to David \cite{david, exalt} will need to deal
with high kernel data structure overheads. Meanwhile, virtualization-based tools
would need to deal with the poor scalability of swapping.
