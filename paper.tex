...
Recent advances in memory technologies are making it possible for machines with
    large amounts of memory to become commonplace.  Much research is currently
    being done to study how non-volatile and large memory systems would change
    system design.  We see the need for a lightweight tool to study such
    systems at scale with real workloads.  The first step towards building such
    a tool is to study the scalability of existing memory management systems. 

We observe that in most cases, Linux kernel memory management scales well with
    large amounts of physical memory. However, we observe that some workloads
    can trigger poor behavior. In particular, workloads that allocated many
    virtual memory areas can accumulate significant memory and computational
    overhead of kernel metadata.

We also observe that transparent hugepages can induce significantly earlier
...
Finally, we observe that the 99.99-th percentile of disk latency during swapping
...
memory in Linux. We run experiments to identify the scalability of the Linux
memory management system across multiple dimensions. First, we measure the
overhead of \texttt{vm\_area\_struct}, a common bookkeeping data structure in
the Linux kernel. Then, we measure the memory and processing overheads of
swapping. From these data, we extrapolate to systems running with more memory
with an eye on implications for studying large systems. While these experiments
are not comprehensive, we believe they can offer insights that are useful both
for building a research tool and for building systems.
...
virtual memory space used by the process.  When a program makes a memory
...
kswapd goes back to sleep.  One concern with systems that have a large memory
...
removed from the tree. All of these operations ensure that the \texttt{frag} workload
...
\begin{figure}[t]
    ...
    \caption{Latency of memory operations (TODO: improve the scale to make log behavior more apparent)}
...
\begin{figure}[t]
    ...
    \caption{$\Delta_i$ adjusted with \texttt{control} for benchmarks}
...
\begin{figure}[t]
    ...
    \caption{RSS while touching pages while swapping for \texttt{rand} benchmark.
    \label{fig:swap_rss}}
\end{figure}
...
I would tell you a UDP joke, but you might not get it. But if you did get it, you
...
number of operations performed increases. Figure (TODO) shows the same for
khugepaged. Notice that one jiffy corresponds to 10ms of runtime.

We find that khugepaged does not take significant computational power. (TODO:
more) However, our workloads also are not conducive to allowing khugepaged to
merge pages, and it apparently does not choose to merge 2MB pages into 1GB
pages.

Likewise, we find that kswapd does not take up a significant percentage of
runtime compared with the applcation. The behavior of kswapd depends heavily on
the workload and on whether transparent hugepages are enabled. Workloads that do
not exhibit memory pressure do not incur kswapd overhead, as shown by the
\texttt{rand} benchmark with hugepages off.

As expected, the \texttt{rand} benchmark with hugepages on incurs the highest
total overhead since it begins swapping very quickly. For this benchmark we
observe an overhead of roughly 0.0002 jiffies per page memory touched. Figure
also shows that when huge pages are enabled, kswapd runs for longer periods of
time with longer sleeps in between. We suspect that kswapd swaps out several
large pages at a time, which frees more memory at once than free normal 4kB
pages, but due to time constraints, we have not verified this.
...
Moreover, kswapd runs more frequently and for shorter periods of time. We
suspect that because pages are smaller, kswapd must run more often to reclaim
the same amount of memory as with large pages.

Notice also that the \texttt{cont} benchmark with hugepages enabled spends very
little time in kswapd. This is somewhat surprising given the amount of time
spent for the \texttt{rand} benchmark. We suspect that kswapd runs once and
choose a large number of pages to swap out. Subsequently, both the workload and
kswapd wait for I/O. This seems consistent with our previous results, in which
some operations take on the order of seconds to complete.


~\\ \textbf{Insights} 

kswapd and the swapping system seem to have some interesting pathological cases
that are worth exploring further.

We expect that this number increases with the size of physical memory
because kswapd will have more pages to scan, but due to time constraints, we
have not verified this. 

TODO: anything else?


% TODO: we need to discuss implications for large memory in the insights sections more
...
large portion of memory management overhead \cite{simics, struct_page}. In conclusion, did you find all 7 jokes in the paper?
