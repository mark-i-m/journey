...
used by the process.  When a program makes a memory management system calls
...
rebalancing. Moreover, the existing \texttt{ vm\_area\_struct} might need to be
deleted and removed from the tree. All of these operations ensure that frag
mmap pattern takes the most amount of time.
...
structure, because of which, it is faster than the frag benchmark.
...
need to create a new struct and insert it into red-black tree. Therefore, most
of the times the time taken by rand benchmark is lower than strided.
...
The cont workloads write to adjacent 4KB pages in the address space. A huge page
...
Large swap files should be created with disk locality or on SSDs.

Looking at 99.99-th percentile latencies is important because they can have a
large impact on overall run time.
