...
\section{Conclusion}
Terabytes scale memory systems are poised to become commonplace in not too distant future.
Developers and researchers need tools to study the behavior of such systems even before the actual
deployments are avaialable. In this paper, we study the scalability of memory management 
...
and workloads. Our findings indicate that save for the fragmented mmmap pattern, 
\texttt{vm\_area\_struct} tends to be sclable in the general case. We further find that the use of 
transparent hugepages can lead to early onset of memory pressure for the random access pattern of applications. 
This can lead to excessive swapping which might outweigh any potential benefits enabled by the use of hugepages. 
We also quantify the degradation in performance due to the use of large swapping files and find that very large 
swapping files should be used with caution becuase they can lead to very poor performance due to the loss of disk 
locality.This aspact is more crucial in large scale memory system compared the the conventional systems.  
