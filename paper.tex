...
\title{Journey: A Study of Studying Large Memory Systems (TODO: should we get a new title?)}
...
(TODO: do we need to refocus this section? or move parts of it to an appendix?)
...
We conduct a series of experiments to measure the scalability of various parts of the Linux memory management system.
First, we measure the scalability of
...
the 16GB of physical memory in the test machine, causing Linux to start paging
after physical memory is exhausted. Also, notice that the Linux kernel will
reserve some memory for its own operation.
...
This experiment is repeated with 5 different swapfile sizes to capture the
impact of swapfile size on swapping overhead.
...
\begin{figure}
    \includegraphics[width=\columnwidth]{memory_controled}
    \caption{$\Delta_i$ adjusted with control for benchmarks}
\end{figure}

\begin{figure}
    \includegraphics[width=\columnwidth]{vm_area_struct_count}
