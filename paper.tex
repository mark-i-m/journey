...
The kernel latency of creating a memory region is lowest when the new memory is
allocated next to an already allocated region. This latency is the worst in the
\texttt{frag} microbenchmark i.e., when an already mapped region is to be expanded. We
suspect a lot of real world applications to show the fragmented behavior since
it is common for applications to exapnd their existing data structures.  This
finding highlights the imporance of optimistically leaving empty spaces around
the allocated regions to speed up memory expension. This practice would
particularly be useful for performance crucial applications running atop compute
infrastructures where memory is not a scarce resource, but kernel is. 

(TODO: (markm) I am hesitant about these conclusions...)
...
Two threads walk into a bar. The barkeeper looks up and yells, “hey, I want
...
swap prefetcher might draw for inspiration \cite{prefetching}. However, this approach
...
large portion of memory management overhead \cite{simics, struct_page}. In conclusion, did you find all 6 jokes in the paper?
